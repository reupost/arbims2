...
/**
 * Class which outputs the html page for RR view
 *
 * @category   DMS
 * @package    SeedDMS
 ...
 * @version    Release: @package_version@
 */
class SeedDMS_View_RR extends SeedDMS_Bootstrap_Style {
	...
	function show() { /* begin function */
		...
		if ($LeftColumnSpan > 0) {
			...
			if ($enableFolderTree) {
				if ($showtree==1){
					$this->contentHeading("<a href=\"../out/out.RR.php?folderid=". $folderid."&showtree=0\"><i class=\"icon-minus-sign\"></i></a>", true);
					$this->contentContainerStart();
?>
		<script language="JavaScript">
		function folderSelected(id, name) {
			window.location = '../out/out.RR.php?folderid=' + id;
		}
		</script>
<?php
					...
					$this->contentHeading("<a href=\"../out/out.RR.php?folderid=". $folderid."&showtree=1\"><i class=\"icon-plus-sign\"></i></a>", true);
				}
		...
		//RR modified to not show folder info to non-admin users
		...
		if ((count($subFolders) > 0)||(count($documents) > 0)){
			...
			print "<thead>\n<tr>\n";
			print "<th></th>\n";	
			print "<th><a href=\"../out/out.RR.php?folderid=". $folderid .($orderby=="n"?"&orderby=s":"&orderby=n")."\">".getMLText("name")."</a></th>\n";
...
//			print "<th>".getMLText("version")."</th>\n";
		...
		//	print "<td><img src=\"images/folder_closed.gif\" width=18 height=18 border=0></td>";
			print "<td><a rel=\"folder_".$subFolder->getID()."\" draggable=\"true\" ondragstart=\"onDragStartFolder(event);\" href=\"out.RR.php?folderid=".$subFolder->getID()."&showtree=".$showtree."\"><img draggable=\"false\" src=\"".$this->imgpath."folder.png\" width=\"24\" height=\"24\" border=0></a></td>\n";
			print "<td><a href=\"out.RR.php?folderid=".$subFolder->getID()."&showtree=".$showtree."\">" . htmlspecialchars($subFolder->getName()) . "</a>";
