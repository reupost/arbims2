...
class SeedDMS_Core_DMS {
	...
	function getUserByLogin($login, $email='') { /* begin function */
		...
		$user = new SeedDMS_Core_User($resArr["id"], $resArr["login"], $resArr["pwd"], $resArr["fullName"], $resArr["email"], $resArr["language"], $resArr["theme"], $resArr["comment"], $resArr["role"], $resArr["hidden"], $resArr["disabled"], $resArr["pwdExpiration"], $resArr["loginfailures"], $resArr["quota"], $resArr["bulletin"], $resArr["linkedaccount"]); //RR added
		$user->setDMS($this);
		return $user;
	} /* end function */
	...
	function getAllUsers($orderby = '') { /* begin function */
		...
		for ($i = 0; $i < count($resArr); $i++) {
			$user = new SeedDMS_Core_User($resArr[$i]["id"], $resArr[$i]["login"], $resArr[$i]["pwd"], $resArr[$i]["fullName"], $resArr[$i]["email"], (isset($resArr["language"])?$resArr["language"]:NULL), (isset($resArr["theme"])?$resArr["theme"]:NULL), $resArr[$i]["comment"], $resArr[$i]["role"], $resArr[$i]["hidden"], $resArr[$i]["disabled"], $resArr[$i]["pwdExpiration"], $resArr[$i]["loginfailures"], $resArr[$i]["quota"], $resArr[$i]["bulletin"], $resArr[$i]["linkedaccount"]); //RR added
			$user->setDMS($this);
			$users[$i] = $user;
		}
