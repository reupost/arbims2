...
if (isset($_COOKIE["mydms_session"])) {
	...
	if ($settings->_sharedLangPath != "") {
		include $settings->_sharedLangPath . "/" . $resArr["language"] . "/lang.inc";
	} else {
...
switch($command) {
	case 'checkpwstrength': /* begin function */
	...
	case 'sessioninfo': /* begin function */
		if($user) {
			echo json_encode($resArr);
		}	
		break; /* end function */

	case 'searchdocument': /* begin function */
		if($user) {
			$query = $_GET['query'];
	...
	case 'searchfolder': /* begin function */
		if($user) {
			$query = $_GET['query'];
	...
	case 'addtoclipboard': /* begin function */
		if($user) {
	...
	case 'movefolder': /* begin function */
		if($user) {
			...
			if($mfolder) {
				if ($mfolder->getAccessMode($user) >= M_READ) {
					if($folder = $dms->getFolder($_REQUEST['targetfolderid'])) {
						if($folder->getAccessMode($user) > M_READWRITE) { /* RR was > */
	...
	case 'movedocument': /* begin function */
		if($user) {
	...
	case 'submittranslation': /* begin function */
