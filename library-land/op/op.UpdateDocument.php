...
*/
        ...
        //RR added to update fulltext index
        if($settings->_enableFullSearch) {
            ...
            if($index) {
                ...
                if(!($hits = $index->find('document_id:'.$document->getId()))) { //should not happen                                  			
                    $index->addDocument(new SeedDMS_Lucene_IndexedDocument($dms, $document, isset($settings->_converters) ? $settings->_converters : null));
                } else {
                    $hit = $hits[0];
