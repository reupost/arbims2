...
class SeedDMS_Core_DMS {
	 ...
	 */
	static function checkIfEqual($object1, $object2) { /* begin function */
	 ...
	 */
	static function filterAccess($objArr, $user, $minMode) { /* begin function */
		if (!is_array($objArr)) {
			return array();
		}
		$newArr = array();
	 ...
	 */
	static function filterUsersByAccess($obj, $users, $minMode) { /* begin function */
		$newArr = array();
	 ...
	 */
	static function filterDocumentLinks($user, $links) { /* begin function */
		$tmp = array();
	 ...
	 */
	function __construct($db, $contentDir) { /* begin function */
		$this->db = $db;
	 ...
	 */
	function getDBVersion() { /* begin function */
	 ...
	 */
	function checkVersion() { /* begin function */
		$tbllist = $this->db->TableList();
		$tbllist = explode(',',strtolower(join(',',$tbllist)));
		if(!array_search('tblversion', $tbllist))
			return true;
	 ...
	 */
	function setRootFolderID($id) { /* begin function */
	 ...
	 * @param interger $id id of root folder
	 */
	function setMaxDirID($id) { /* begin function */
		$this->maxDirID = $id;
	} /* end function */
	 ...
	 */
	function getRootFolder() { /* begin function */
		if(!$this->rootFolderID) return false;
		return $this->getFolder($this->rootFolderID);
	} /* end function */

	function setEnableConverting($enable) { /* begin function */
		$this->enableConverting = $enable;
	} /* end function */

	function setConvertFileTypes($types) { /* begin function */
		$this->convertFileTypes = $types;
	} /* end function */

	function setViewOnlineFileTypes($types) { /* begin function */
	 ...
	 */
	function login($username, $password) { /* begin function */
	} /* end function */
	 ...
	 *
	 */
	function setUser($user) { /* begin function */
		$this->user = $user;
	} /* end function */
	 ...
	 */
	function getDocument($id) { /* begin function */
		...
		$document = new SeedDMS_Core_Document($resArr["id"], $resArr["name"], $resArr["comment"], $resArr["date"], $resArr["expires"], $resArr["owner"], $resArr["folder"], $resArr["inheritAccess"], $resArr["defaultAccess"], $lock, $resArr["keywords"], $resArr["sequence"], $resArr["linkedgislayer"], $resArr["linkedgisfeature"]); //RR added linkedgis entries
	 ...
	 */
	function getDocumentsByUser($user) { /* begin function */
	 ...
	 * @return array list of documents
	 */
	function getDocumentsLockedByUser($user) { /* begin function */
	 ...
	 */
	function getDocumentByName($name, $folder=null) { /* begin function */
	 ...
	 */
	function getDocumentContent($id) { /* begin function */
		if (!is_numeric($id)) return false;
	...
	function makeTimeStamp($hour, $min, $sec, $year, $month, $day) { /* begin function */
	...
	function search($query, $limit=0, $offset=0, $logicalmode='AND', $searchin=array(), $startFolder=null, $owner=null, $status = array(), $creationstartdate=array(), $creationenddate=array(), $modificationstartdate=array(), $modificationenddate=array(), $categories=array(), $attributes=array(), $mode=0x3, $expirationstartdate=array(), $expirationenddate=array()) { /* begin function */
		...
		if($mode & 0x1) {
			...
			// Check to see if the search has been restricted to a particular
			// attribute.
            //RR: multivalue attributes don't work since $attribute = 'Array()', even if its a single item.  Searching with more than one value would also be quite a large change.
            //RR so approach for now is to modify the search interface to show a drop-down (i.e. pick one) for all attributes, and not the multi-selection list
            //note that multi-attributes are preceded by the separator character, single-select ones are not.
            //multi-select entries can have multiple values in the list so its a 'like' clause
            //TODO: because of 'like' stmt, if you have attribute value "xxx" and another value "xxxyyy", both will be matched - it matches all starting with the criterion
				...
				foreach($attributes as $attrdefid=>$attribute) {
					if($attribute) {
						...
						if($attrdef->getObjType() == SeedDMS_Core_AttributeDefinition::objtype_document) {
                            if (!$attrdef->getMultipleValues())  //RR was below
							//if($attrdef->getValueSet()) {
								$searchAttributes[] = "`tblDocumentAttributes`.`attrdef`=".$attrdefid." AND `tblDocumentAttributes`.`value`='".$attribute."'";
                            else
								$searchAttributes[] = "`tblDocumentAttributes`.`attrdef`=".$attrdefid." AND `tblDocumentAttributes`.`value` like '%" . $attrdef->getSeparator() . $attribute."%'";
							...
							//if($attrdef->getValueSet())
								...
								$searchAttributes[] = "`tblDocumentContentAttributes`.`attrdef`=".$attrdefid." AND `tblDocumentContentAttributes`.`value` like '%" . $attrdef->getSeparator() . $attribute."%'";
						}
					}
				}
			}
			...
			if ($searchAttributes) {
                //RR modified to fix multiple attribute criteria causing no search results
                if (count($searchAttributes) == 1) {
                    $searchQuery .= " AND (".implode(" AND ", $searchAttributes).")";
                } else {
                    //RR example for two attributes:
                    //SELECT tda0.document from 
                    //(SELECT document FROM tblDocumentAttributes WHERE `tblDocumentAttributes`.`attrdef`=3 AND `value`='Val1') tda0 JOIN
                    //(SELECT document FROM tblDocumentAttributes WHERE `tblDocumentAttributes`.`attrdef`=1 AND `tblDocumentAttributes`.`value` like '%#Climate change%') tda1 ON tda1.document = tda0.document;
                    //RR note - this probably won't work well with large numbers of attribute criteria!
                    $searchQuery .= " AND `tblDocumentAttributes`.document IN (SELECT tda0.document FROM ";
                    $i = 0;
                    foreach ($searchAttributes as $attr) {
                        $searchQuery .= "(SELECT document from `tblDocumentAttributes` WHERE " . $attr . ") tda" . $i;
                        if ($i > 0) $searchQuery .= " ON tda" . $i . ".document = tda0.document";
                        $searchQuery .= " JOIN ";
                        $i++;
                    }
                    //remove superfluous final 'JOIN'
                    $searchQuery = substr($searchQuery, 0, -1*strlen(" JOIN "));
                    $searchQuery .= ")";
	 ...
	 */
	function getFolder($id) { /* begin function */
		if (!is_numeric($id)) return false;
	 ...
	 */
	function getFolderByName($name, $folder=null) { /* begin function */
		if (!$name) return false;
	 ...
	 */
	function checkFolders() { /* begin function */
	 ...
	 * @return array list of errors
	 */
	function checkDocuments() { /* begin function */
		$queryStr = "SELECT * FROM tblFolders";
		$resArr = $this->db->getResultArray($queryStr);
	 ...
	 */
	function getUser($id) { /* begin function */
		if (!is_numeric($id))
			return false;
		...
		$user = new SeedDMS_Core_User($resArr["id"], $resArr["login"], $resArr["pwd"], $resArr["fullName"], $resArr["email"], $resArr["language"], $resArr["theme"], $resArr["comment"], $resArr["role"], $resArr["hidden"], $resArr["disabled"], $resArr["pwdExpiration"], $resArr["loginfailures"], $resArr["quota"], $resArr["bulletin"], $resArr["linkedaccount"], $resArr["mastermoderator"]); //RR added bulletin, is_linked_account
		$user->setDMS($this);
		return $user;
	} /* end function */
	 ...
	 * @return object instance of {@link SeedDMS_Core_User} or false
	 */
	function getUserByLogin($login, $email='') { /* begin function */
		...
		$user = new SeedDMS_Core_User($resArr["id"], $resArr["login"], $resArr["pwd"], $resArr["fullName"], $resArr["email"], $resArr["language"], $resArr["theme"], $resArr["comment"], $resArr["role"], $resArr["hidden"], $resArr["disabled"], $resArr["pwdExpiration"], $resArr["loginfailures"], $resArr["quota"], $resArr["bulletin"], $resArr["linkedaccount"], $resArr["mastermoderator"]); //RR added
		$user->setDMS($this);
		return $user;
	} /* end function */
	 ...
	 * @return object instance of {@link SeedDMS_Core_User} or false
	 */
	function getUserByEmail($email) { /* begin function */
	 ...
	 */
	function getAllUsers($orderby = '') { /* begin function */
		if($orderby == 'fullname')
		...
		for ($i = 0; $i < count($resArr); $i++) {
			$user = new SeedDMS_Core_User($resArr[$i]["id"], $resArr[$i]["login"], $resArr[$i]["pwd"], $resArr[$i]["fullName"], $resArr[$i]["email"], (isset($resArr["language"])?$resArr["language"]:NULL), (isset($resArr["theme"])?$resArr["theme"]:NULL), $resArr[$i]["comment"], $resArr[$i]["role"], $resArr[$i]["hidden"], $resArr[$i]["disabled"], $resArr[$i]["pwdExpiration"], $resArr[$i]["loginfailures"], $resArr[$i]["quota"], $resArr[$i]["bulletin"], $resArr[$i]["linkedaccount"], $resArr[$i]["mastermoderator"]); //RR added
			$user->setDMS($this);
			$users[$i] = $user;
		}
	 ...
	 */
	function addUser($login, $pwd, $fullName, $email, $language, $theme, $comment, $role='0', $isHidden=0, $isDisabled=0, $pwdexpiration='', $isBulletin = 1) { /* begin function */
		$db = $this->db;
	 ...
	 */
	function getGroup($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	 ...
	 * @return object/boolean group or false if no group was found
	 */
	function getGroupByName($name) { /* begin function */
	 ...
	 */
	function getAllGroups() { /* begin function */
	 ...
	 *         case of an error.
	 */
	function addGroup($name, $comment) { /* begin function */
	...
	function getKeywordCategory($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	...
	function getKeywordCategoryByName($name, $userID) { /* begin function */
	...
	function getAllKeywordCategories($userIDs = array()) { /* begin function */
	 ...
	 */
	function getAllUserKeywordCategories($userID) { /* begin function */
		$queryStr = "SELECT * FROM tblKeywordCategories";
		if ($userID != -1)
	...
	function addKeywordCategory($userID, $name) { /* begin function */
	...
	function getDocumentCategory($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	...
	function getDocumentCategories() { /* begin function */
    ...
    function getGISlayers() { /* begin function */
		...
		$layers = array();
		foreach ($resArr as $row) {
			$lay = new SeedDMS_Core_GISlayer($row["id"], $row["displayname"]);
			$lay->setDMS($this);
    ...
    function getGISfeatures($layerid = 0) { /* begin function */
        if ($layerid > 0) {
            $queryStr = "SELECT * FROM tblgislayer_feature WHERE gislayer_id = " . $layerid;
        } else {
		...
		$features = array();
		foreach ($resArr as $row) {
			$feat = new SeedDMS_Core_GISfeature($row["fid"], $row["gislayer_id"], $row["attributes_concat"], $row["description_text"]);
			$feat->setDMS($this);
	 ...
	 */
	function getDocumentCategoryByName($name) { /* begin function */
		if (!$name) return false;
	...
	function addDocumentCategory($name) { /* begin function */
	 ...
	 */
	function getNotificationsByGroup($group, $type=0) { /* begin function */
	 ...
	 * @return array array of notifications
	 */
	function getNotificationsByUser($user, $type=0) { /* begin function */
		$queryStr = "SELECT `tblNotify`.* FROM `tblNotify` ".
	 ...
	 */
	function createPasswordRequest($user) { /* begin function */
	 ...
	 * @param string $hash
	 */
	function checkPasswordRequest($hash) { /* begin function */
	 ...
	 * @param string $hash
	 */
	function deletePasswordRequest($hash) { /* begin function */
	 ...
	 */
	function getAttributeDefinition($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	 ...
	 * @return object instance of {@link SeedDMS_Core_AttributeDefinition} or false
	 */
	function getAttributeDefinitionByName($name) { /* begin function */
		if (!$name) return false;
	 ...
	 */
	function getAllAttributeDefinitions($objtype=0) { /* begin function */
	 ...
	 * @return object of {@link SeedDMS_Core_User}
	 */
	function addAttributeDefinition($name, $objtype, $type, $multiple=0, $minvalues=0, $maxvalues=1, $valueset='', $regex='') { /* begin function */
	 ...
	 */
	function getAllWorkflows() { /* begin function */
	 ...
	 */
	function getWorkflow($id) { /* begin function */
	 ...
	 * @return object of instances of {@link SeedDMS_Core_Workflow} or false
	 */
	function getWorkflowByName($name) { /* begin function */
		if (!$name) return false;
	...
	function addWorkflow($name, $initstate) { /* begin function */
		$db = $this->db;
	 ...
	 */
	function getWorkflowState($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	 ...
	 */
	function getWorkflowStateByName($name) { /* begin function */
		if (!$name) return false;
	 ...
	 */
	function getAllWorkflowStates() { /* begin function */
		$queryStr = "SELECT * FROM tblWorkflowStates ORDER BY name";
		$ressArr = $this->db->getResultArray($queryStr);
	 ...
	 */
	function addWorkflowState($name, $docstatus) { /* begin function */
		$db = $this->db;
	 ...
	 */
	function getWorkflowAction($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	 ...
	 * @return object instance of {@link SeedDMS_Core_Workflow_Action} or false
	 */
	function getWorkflowActionByName($name) { /* begin function */
		if (!$name) return false;
	 ...
	 */
	function getAllWorkflowActions() { /* begin function */
	 ...
	 */
	function addWorkflowAction($name) { /* begin function */
		$db = $this->db;
	 ...
	 */
	function getWorkflowTransition($id) { /* begin function */
		if (!is_numeric($id))
			return false;
	 ...
	 */
	function getUnlinkedDocumentContent() { /* begin function */
	 ...
	 */
	function getNoFileSizeDocumentContent() { /* begin function */
	 ...
	 */
	function getNoChecksumDocumentContent() { /* begin function */
	 ...
	 */
	function getStatisticalData($type='') { /* begin function */
		switch($type) {
			case 'docsperuser':
	...
	/*** RR TODO description */
	//RR added
	function getDashboardListing($list = 'mostdownloaded', $num = 5) { /* begin function */
		
        $queryStr = "";
		if ($list == 'mostdownloaded') {		
			$queryStr = "SELECT dd.documentid, d.name, ds.latestversion, count(*) as numdownloads FROM (tbldocumentdownloads dd JOIN tbldocuments d ON dd.documentid = d.id) JOIN (SELECT documentID, max(version) as latestversion FROM tbldocumentstatus GROUP BY documentID) ds ON dd.documentID = ds.documentID GROUP BY dd.documentID ORDER BY count(*) DESC, d.name ASC LIMIT 0," . $num;
		}
        if ($list == 'mostrecent') {
            $queryStr = "SELECT d.id as documentid, d.name, ds.latestversion, d.date FROM tbldocuments d JOIN (SELECT documentID, max(version) as latestversion FROM tbldocumentstatus GROUP BY documentID) ds ON d.id = ds.documentID ORDER BY d.date DESC, d.name ASC LIMIT 0," . $num;
        }
        if ($list == 'recommended') {
            $queryStr = "SELECT d.id as documentid, d.name, ds.latestversion FROM ((tbldocuments d JOIN (SELECT documentID, max(version) as latestversion FROM tbldocumentstatus GROUP BY documentID) ds ON d.id = ds.documentID) JOIN tbldocumentcategory dc ON d.id = dc.documentID) JOIN tblcategory c ON c.id = dc.categoryID WHERE c.name = 'Recommended' ORDER BY d.name ASC LIMIT 0," . $num;
        }
        if ($list == 'mostrated') {
            $queryStr = "SELECT dr.documentid, d.name, ds.latestversion, sum(dr.rating) as docrating FROM (tbldocumentrating dr JOIN tbldocuments d ON dr.documentid = d.id) JOIN (SELECT documentID, max(version) as latestversion FROM tbldocumentstatus GROUP BY documentID) ds ON dr.documentID = ds.documentID GROUP BY dr.documentID ORDER BY sum(dr.rating) DESC, d.name ASC LIMIT 0," . $num;
        }
        if ($queryStr > "") {
            $resArr = $this->db->getResultArray($queryStr);
			if (is_bool($resArr) && !$resArr)
	 ...
	 *        callback
	 */
	function setCallback($name, $func, $params=null) { /* begin function */
		if($name && $func)
