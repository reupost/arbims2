...
function formatted_size($size_bytes) { /* begin function */
...
function sanitizeString($string) { /* begin function */
 ...
 * older versions
 */
function mydmsDecodeString($string) { /* begin function */
...
function createVersionigFile($document) { /* begin function */
	global $settings, $dms;
...
//RR added since stat (used in dskspace below does not work on Windows systems
...
function dskspace($dir) { /* begin function */
   $s = stat($dir);
   $space = $s["blocks"]*512;
   if (is_dir($dir)) {
     $dh = opendir($dir);
...
function add_log_line($msg="") { /* begin function */
	global $logger, $user;
...
function _add_log_line($msg="") { /* begin function */
...
function showtree() { /* begin function */
	global $settings;
 ...
 */
function createFormKey($formid='') { /* begin function */
 ...
 */
function createHiddenFieldWithKey($formid='') { /* begin function */
 ...
 */
function checkFormKey($formid='', $method='POST') { /* begin function */
	switch($method) {
		case 'GET':
 ...
 */
function checkQuota() { /* begin function */
...
/**
 * Format delimited string (where first char is delimiter)
 *
 * @return string
 *
 * RR added 
 */
 //RR
function prettyList($delimStr, $replaceWith) {	
	$delim = $delimStr[0];
	$cleantext = substr($delimStr, 1);
	$cleantext = str_replace($delim, $replaceWith, $cleantext);
	return $cleantext;
}
?>
