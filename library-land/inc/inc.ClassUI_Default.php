...
class UI_Default {
	...
	static function getStyles() { /* begin function */
		global $settings;
	...
	function htmlStartPage($title="", $bodyClass="", $customJS = "") { /* begin function */ //RR mod for custom JS
	...
	function folderNavigationBar($folder) { /* begin function */
		...
		if ($accessMode == M_READ && !$user->isGuest()) {
			echo "<li id=\"first\"><a href=\"../out/out.FolderNotify.php?folderid=". $folderID ."&showtree=".showtree()."\">".getMLText("edit_folder_notify")."</a></li>\n";
                ...
                /* RR modified for latest permisions: users to be able to add docs anywhere, but not add folders */
                /* RR this does not seem to be called in the actual code.  Looking for another implementation... */
		else if ($accessMode >= M_READWRITE) {
                        if ($accessMode > M_READWRITE) /* RR */ 
			...
			if ($settings->_enableLargeFileUpload && $accessMode > M_READWRITE) /* RR added accemode criterion */
	...
	function documentNavigationBar()	{ /* begin function */
		...
		if ($accessMode == M_ALL) {
                    if ($user->isAdmin()) { /* RR added - doc owners cant modify the settings below */
	...
	function accountNavigationBar() { /* begin function */
	...
	function myDocumentsNavigationBar() { /* begin function */
	...
	function adminToolsNavigationBar() { /* begin function */
	...
	function calendarNavigationBar($d){ /* begin function */
	...
	function contentContainerStart() { /* begin function */
...
<?php
	...
	static function exitError($pagetitle,$error) { /* begin function */
	...
	function printFoldersTree($accessMode, $exclude, $folderID, $currentFolderID=-1, $navigation=false) {	/* begin function */
