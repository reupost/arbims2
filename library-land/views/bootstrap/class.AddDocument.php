...
class SeedDMS_View_AddDocument extends SeedDMS_Bootstrap_Style {
    ...
    function show() { /* begin function */
            ...
            ?>
                    if (!document.form1.name.disabled) {
                        if (document.form1.name.value == "")
                            msg.push("<?php printMLText("js_no_name"); ?>");
                    }
                    if (document.form1.comment.value == "")
                        msg.push("<?php printMLText("js_no_comment"); ?>");
                    if (document.form1.keywords.value == "")
                        msg.push("<?php printMLText("js_no_keywords"); ?>");
            <?php
        }
        ?>
                if (msg != "") {
        ...
        <?php /* RR added */ ?>
                <tr>
                    <td colspan="2"><b><?php printMLtext("linked_map") ?></b></td>            
                </tr>
                <tr>
                    <td><?php printMLText('linked_map_layer') ?>:</td>
                    <td>
                        <select class="chzn-select" id="linkedgislayer" name="linkedgislayer" data-placeholder="<?php printMLText('select_map_layer'); ?>" data-no_results_text="<?php printMLText('unknown_map_layer'); ?>">
        <?php
        $layers = $dms->getGISlayers();
        echo "<option value=\"0\"";
        echo ">-" . getMLtext('none') . "-</option>";
        foreach ($layers as $lay) {
            echo "<option value=\"" . $lay->getID() . "\"";
            echo ">" . getMLtext($lay->getName()) . "</option>";
        }
        ?>
                        </select>
                    </td>
                ...
                <tr>
                    <td><?php printMLText('linked_map_feature') ?>:</td>
                    <td>
                        <select class="chzn-select" id="linkedgisfeature" name="linkedgisfeature" data-placeholder="<?php printMLText('select_map_feature'); ?>" data-no_results_text="<?php printMLText('unknown_map_feature'); ?>">
            ...
            foreach ($docAccess["users"] as $usr) {
                ...
                foreach ($res as $r)
                    if ($r['reviewerUserID'] == $usr->getID())
                        $mandatory = true;
                //RR modified to show mandatory reviewers in plain text above list; also removed login details of reviewer
                if ($mandatory) {
                    //$select_text .= "<option disabled=\"disabled\" value=\"".$usr->getID()."\">". htmlspecialchars($usr->getLogin()." - ".$usr->getFullName())."</option>";
                    $select_text .= "<option disabled=\"disabled\" value=\"" . $usr->getID() . "\">" . htmlspecialchars($usr->getFullName()) . "</option>";
                    $mandatory_text .= htmlspecialchars($usr->getFullName()) . " ";
                } else {
                    $select_text .= "<option value=\"" . $usr->getID() . "\">" . htmlspecialchars($usr->getFullName()) . "</option>";
                }
            }
            if ($mandatory_text > "") { //RR
                printMLText("mandatory_reviewer");
                echo ": " . trim($mandatory_text) . "<br/>";
            } else { /* RR else added to prevent selection of reviewers if mandatory reviewer defined */
            ?>
                    ...
                    <tr>
                        ...
                        <td>
                                ...
                                foreach ($docAccess["groups"] as $grp) {
                                    ...
                                    foreach ($res as $r)
                                        if ($r['reviewerGroupID'] == $grp->getID())
                                            $mandatory = true;
                                    //RR TODO add 'mandatory group' in plain text
                                    if ($mandatory)
                                        print "<option value=\"" . $grp->getID() . "\" disabled=\"disabled\">" . htmlspecialchars($grp->getName()) . "</option>";
                                    else
                            ...
                            ?>
                <?php } /* from mandatory reviewer section */ ?>
                        </td>
                    </tr>
                    ...
                    <tr>	
                        ...
                        <td>
                            ...
                            foreach ($docAccess["users"] as $usr) {
                                ...
                                if ($mandatory) { //RR mod below 
                                    $select_text .= "<option value=\"" . $usr->getID() . "\" disabled='disabled'>" . htmlspecialchars($usr->getFullName()) . "</option>";
                                    $mandatory_text .= htmlspecialchars($usr->getFullName()) . " ";
                                } else {
                                    $select_text .= "<option value=\"" . $usr->getID() . "\">" . htmlspecialchars($usr->getLogin() . " - " . $usr->getFullName()) . "</option>";
                                }
                            }
                            if ($mandatory_text > "") { //RR
                                printMLText("mandatory_approver");
                                echo ": " . trim($mandatory_text) . "<br/>";
                            } else { /* if mandatory approver, dont allow choice */
                            ?>
                        ...
                        <td>
                            <select class="chzn-select span9" name="grpApprovers[]" multiple="multiple" data-placeholder="<?php printMLText('select_grp_approvers'); ?>">
                                ...
                                foreach ($docAccess["groups"] as $grp) {
                                    ...
                                    $mandatory = false;
                                    //RR TODO add mandatory groups in plain text (and add to lang files)
                                    foreach ($res as $r)
