...
class SeedDMS_View_ViewDocument extends SeedDMS_Bootstrap_Style {
	...
	function getAccessModeText($defMode) { /* begin function */
		switch($defMode) {
			case M_NONE:
	...
	function printAccessList($obj) { /* begin function */
	...
	function show() { /* begin function */
        ...
        $attrdefs = $this->params['attrdefs']; //RR added
...
?>
        ...
        //RR modified to display all attributes regardless whether document has value set for them
        if($attrdefs) {
			foreach($attrdefs as $attrdef) {	
                $attr_defn_id = $attrdef->getID();
?>
		    <tr>
					...
					<td><?php 
                            if($attributes) {
                                foreach($attributes as $attribute) {
                                    $attr_with_value = $attribute->getAttributeDefinition();
                                    $attr_with_value_id = $attr_with_value->getID();
                                    if ($attr_with_value_id == $attr_defn_id) {
                                    
                                    //RR modified to remove delimiting character when displaying multiple-value entries
                                        if ($attrdef->getMultipleValues()) {
                                            echo prettyList(htmlspecialchars($attribute->getValue()),"<br>");
                                        } else {
                                            echo htmlspecialchars($attribute->getValue()); //RR original display 
                                        }
                                        break;
                                    }
                                }
...
?>
          ...
          <?php /* RR added */ ?>
          <li><a data-target="#map" data-toggle="tab"><?php printMLText('linked_map'); ?></a></li>
          <?php /* RR added end */ ?>
		</ul>
...
<?php
		...
		*/
        ...
        //RR added
        $usersRating = $document->getUsersRating($user->getId());
        print "<li><b><i>". getMLText("rating",array('number'=>$document->getRating())) . ($usersRating > 0? " (" . getMLText("rated_already") . ")" : "") . "</i></b></li>";
        //RR added end
        ...
        //RR added
        if($accessop->mayRate()) {
            print "<li><a href=\"../op/op.DocumentRate.php?documentid=".$documentid."\"><img src='images/icons/thumbs-up-icon.png' height='16px' width='16px'> ".getMLText("rate")."</a></li>";
        }
        //RR added end
		print "</ul>";
		echo "</td>";
...
<?php       
		$this->contentContainerStart();
        $linkedlayer = $document->getLinkedGISlayer();
        if ($linkedlayer > '') {
            print "<table class=\"table table-condensed\">";
            print "<tbody>\n";
            print "<tr><td><b>" . getMLtext('linked_map_layer'). ":</b></td><td><b>" . getMLtext($linkedlayer) . "</b></td></tr>";
            $linkedfeature = $document->getLinkedGISfeature();
            if ($linkedfeature > '') {
                echo "<tr><td><b>" . getMLtext('linked_map_feature'). ":</b></td><td>" . $linkedfeature . "</td></tr>";
            }
            print "</tbody>";
            print "</table>";
        } else {
            printMLtext('linked_map_none');
        }		
         //TODO nice-to-have: display map with this feature highlighted
		$this->contentContainerEnd();
?>
		  </div>
           <?php /* RR added end */ ?> 
