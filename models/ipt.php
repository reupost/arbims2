...
//returns -1 on success, 0 on failure
    function ProcessDwCArchive($datasetid, $archive)
    {
        ...
        $res3 = -1;
        if ($thefile != 'eml.xml') {
            $res3 = $this->ProcessDwCRecords($archive, $datasetid, $errormsg);
...
//returns -1 on success, 0 on failure
    function UpdateIPTResourcesStatus($iptRSS = "")
    {
        ...
        foreach ($rss->items as $item) {
            ...
            $resourcestatus = $this->GetIPTResourceStatus($link, $date_timestamp);
            ...
            if ($resourcestatus == 1) { //new
                ...
                if (!$res) {
                    $OUTPUT .= "SQL error in UpdateIPTResourcesStatus";
                }
            }
        }
...
//NOTE: composite primary keys are not accommodated here
    function GetPrimaryKeyFirstField($table)
    {
        ...
        if (!$result) {
            echo "SQL error in GetPrimaryKeyFirstField";
            exit;
        } //error
...
//return -1 on success, 0 on any failures
    function ImportDwCData($filename)
    {
        ...
        if (!$result) {
            echo "SQL error in ImportDwCData";
            exit;
        } //error
...
//returns -1 on success, 0 on faiulre
    ...
    public function UpdateIPTResources($iptRSS = "")
    {
        global $DEBUGGING;
        if ($DEBUGGING) {
        ...
        $res = $this->UpdateIPTResourcesStatus($iptRSS); //updates the table of IPT resources where addedtoportal = false means it needs to be reprocessed
        if (!$res) {
            $OUTPUT .= getMLtext("ipt_sync_fail") + "<br/>";
            $this->UnsetSemaphor();
            return 0;
        }
        $recreatesummaries = $this->RemoveObsoleteResources();
        $result = pg_query_params("SELECT * FROM dataset WHERE (addedtoportal = false)", array());
        if (!$result) {
            $OUTPUT .= "<br/><br/>SQL error in UpdateIPTResources";
            return 0;
        }
        while ($row = pg_fetch_array($result)) {
            ...
            $localfile = $this->DownloadIPTArchive($row['dwca'], $row['eml']); //get the file: either DWCA zip file or EML.XML
            if ($localfile != "") {
                $res = $this->ProcessDwCArchive($row['datasetid'], $localfile); //unpack file, put metadata into datasetmetadata table, put dwc table(s) into limbo schema
                if (!$res) {
                    $OUTPUT .= getMLtext("ipt_sync_error_unzip") . "<br/>";
                } else {
                    $this->PopulateDatasetCleanMetadata($row['datasetid']);
                    $res = $this->ImportDwCData($localfile); //now parse across into main DB and fix up geometry
                    if (!$res) {
                ...
                $OUTPUT .= getMLtext("ipt_sync_error_download") . "<br/>";
            }
        }
