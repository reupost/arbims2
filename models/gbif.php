...
"predicates": [
    ...
    private function AddGBIFDownloadToQueue($gbif_id, $gbif_metadata) {
        ...
        if (array_key_exists("status", $metadata_json) && array_key_exists("downloadLink", $metadata_json)) {
            ...
            if (!$res) return "Error inserting download " . $gbif_id . " into queue";
            return "Download " . $gbif_id . " added to queue";
        } else {
            return "Error inserting download " . $gbif_id . " into queue - bad GBIF metadata";
        }
    }
    ...
    private function log($text) {
        ...
        file_put_contents('logs/log_'.date("Y-m-d").'.txt', $timestamp . ' ' . $text . PHP_EOL, FILE_APPEND);
    }

    public function InitiateGBIFDownload() {
        //TODO: set up task to run this every week
        global $siteconfig;
        ...
        if (!$still_busy) {
            ...
            $downloaded_id = -1;
            try {
                $downloaded_id = $this->RetrieveCompleteDownloads();
            } catch (Exception $e) {
                echo 'Caught exception: ',  $e->getMessage(), "\n";
            }

            if ($downloaded_id >= 0) {
                return 'Download #' . $downloaded_id . ' successfully retrieved';
            } else {
    ...
    public function RetrieveCompleteDownloads() {
        ...
        while (($row = pg_fetch_array($res)) && !$download_id) {
            ...
            curl_close($ch);
            fclose($fp);
            //determine if success by testing unzip
