...
class SingleDataset {
    ...
    public function SingleDataset($datasetkey) {
        $this->datasetkey = $datasetkey;
    }
    
    public function GetViewableAttributes() {
        return array("title", "link", "dwca", "eml", "pubdate", "_creator", "_creator_org", "_contact", "_contact_org", "_keywords", "_citation", "region_albertine", "region_mountains", "region_lakes");
    }
    
    public function GetAttributes() {        
        $attribs = array();
        
        $res = pg_query_params("SELECT *, _regions[1] as region_albertine, _regions[2] as region_mountains, _regions[3] as region_lakes FROM dataset WHERE datasetkey = $1", array($this->datasetkey));
        if (!$res) return $attribs; //error
        
        $row = pg_fetch_array($res, null, PGSQL_ASSOC);
        if (!$row) return $attribs; //no dataset with that datasetkey
    ...
    public function SetAttributes($data, &$save_msg) {              
        $res = pg_query_params("SELECT * FROM dataset WHERE datasetkey = $1", array($data['datasetkey']));
        if (!$res) { $save_msg = getMLtext('sql_error'); return 0; }
        $row = pg_fetch_array($res, null, PGSQL_ASSOC);
        if (!$row) { $save_msg = getMLtext('save_dataset_unknown', array('datasetkey' => $data['datasetkey'])); return 0; } //invalid datasetkey

        /*
        if ($data['color'] != '') {
            if (!preg_match('/^#[a-f0-9]{6}$/i', $data['color'])) {
                $save_msg = getMLtext('save_dataset_invalid_color', array('datasetkey' => $data['datasetkey']));
                return 0; 
            } //invalid color hex
        } else { */
            $data['color'] = "000000"; //deprecated
        //}
        $regions = "{";
        ...
        $save_err = 0;
        $res = pg_query_params("UPDATE dataset SET _regions = $1 WHERE datasetkey = $2", array($regions, $data['datasetkey']));
        if (!$res) return 0;    
        ...
        if ($row['_has_occurrence'] == 't') {
            ...
            //if (!$res) $save_err = 1;
            $res = pg_query_params("UPDATE vw_occ_list1 SET _regions = $1 WHERE datasetkey = $2", array($regions, $data['datasetkey']));
            if (!$res) $save_err = 1;        
