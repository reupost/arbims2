...
// Updated Reuben Roberts April 2014: added various "isset" checks and initialising array elements
...
//print '_characterData ' . $this->element . ':' . $data . "\n";
    ...
    //print 'abstract=' . $test . '<br/>';
						if (!isset($this->layer_props[ $this->layer_level ][ $this->element ])) $this->layer_props[ $this->layer_level ][ $this->element ] = ""; //RR added to initialise array element
            ...
            if( $this->layer_level > 0 && ! $this->inSomething() )
            {
                switch( $this->element )
                {
                    ...
                    case 'Name':
                        // if not a style name!!
						if (!isset($this->layer_props[ $this->layer_level][ $this->element ])) $this->layer_props[ $this->layer_level][ $this->element ] = ""; //RR initialise
                        $this->layer_props[ $this->layer_level][ $this->element ] .= $text;
                        break;
                    case 'Abstract':
    //print 'abstract=' . $test . '<br/>';
                    ...
                    case 'SRS':
                        ...
                        && $this->layer_props[ $this->layer_level][ $this->element ] != '' )
                        {
							...
							$this->layer_props[ $this->layer_level][ $this->element ] = ""; //RR initialise
						}
                        $this->layer_props[ $this->layer_level][ $this->element ] .= $text;
                        break;
                }
...
//print '_endElement ' . $name . "\n";
        ...
        {
            case 'Layer':
                if(!isset($this->layer_props[ $this->layer_level ]['layer_id']) || ! $this->layer_props[ $this->layer_level ]['layer_id'] ) //RR added isset check
                {
                    $this->layer_props[ $this->layer_level ]['layer_id'] = $this->numLayers++;
                    if( $this->cbLayer )
                        eval( $this->cbLayer . "(\$this->layer_props[\$this->layer_level], \$this->cbLayerParams );" );
