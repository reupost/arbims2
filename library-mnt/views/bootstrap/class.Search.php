...
class SeedDMS_View_Search extends SeedDMS_Bootstrap_Style {
	 ...
	 */
	function markQuery($str, $tag = "b") { /* begin function */
...
<tr>
	...
	<td><?php 
            //RR was (now changed to only show dropdowns for search on multi-select attributes
            //$this->printAttributeEditField($attrdef, isset($attributes[$attrdef->getID()]) ? $attributes[$attrdef->getID()] : ''); 
            $this->printAttributeSingleSelect($attrdef, isset($attributes[$attrdef->getID()]) ? $attributes[$attrdef->getID()] : '');
        ?>
    </td>
</tr>
<?php
				...
				print "<br /><span style=\"font-size: 85%; font-style: italic; color: #666; \">".getMLText('owner').": <b>".htmlspecialchars($owner->getFullName())."</b>, ".getMLText('creation_date').": <b>".date('Y-m-d', $document->getDate())."</b>, ".getMLText('version')." <b>".$version."</b> - <b>".date('Y-m-d', $lc->getDate())."</b></span>";
					...
					if($lcattributes) {
						foreach($lcattributes as $lcattribute) {
							...
							print "<li>".htmlspecialchars($attrdef->getName()).": "; //RR modified to print list nicely
							if ($attrdef->getMultipleValues()) {
								echo prettyList(htmlspecialchars($lcattribute->getValue()),"; ");								
							} else {
								print htmlspecialchars($lcattribute->getValue());
							}
							print "</li>\n";
						}
					}
					...
					if($docttributes) {
						foreach($docttributes as $docttribute) {
							...
							$attrdef = $docttribute->getAttributeDefinition(); //RR modified to print list nicely
							print "<li>".htmlspecialchars($attrdef->getName()).": ";
							if ($attrdef->getMultipleValues()) {
								echo prettyList(htmlspecialchars($docttribute->getValue()),"; ");								
							} else {
								print htmlspecialchars($docttribute->getValue());
							}							
							print "</li>\n";
						}
					}
...
<?php
					...
					if($folderattributes) {
						foreach($folderattributes as $folderattribute) {
							...
							if ($attrdef->getMultipleValues()) { //RR pretty list print
								echo prettyList(htmlspecialchars($folderattribute->getValue()),"; ");								
							} else {
								print htmlspecialchars($folderattribute->getValue());
							}							
							print "</li>\n";
						}
					}
