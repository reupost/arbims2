...
class SeedDMS_View_EditDocument extends SeedDMS_Bootstrap_Style {
	...
	function show() { /* begin function */
        ...
        //$customJS = "var MapFeatureURL = \"" . getMLtext('linked_map_feature_pick_layer_first') . "\";";        
...
{
	msg = new Array();
	if (document.form1.name.value == "") msg.push("<?php printMLText("js_no_name");?>"); //RR fix
<?php
	if ($strictformcheck) {
	?>
...
?>
        ...
        <?php
            ...
            echo "<option value=\"0\"";
            if ($document->getLinkedGISlayer() == '') echo " selected";
            echo ">-" . getMLtext('none') . "-</option>";
			foreach($layers as $lay) {
				echo "<option value=\"".$lay->getID()."\"";
				if($lay->getID() == $document->getLinkedGISlayerID())
					echo " selected";
        ...
        <?php
			$features = $dms->getGISfeatures($document->getLinkedGISlayerID());
            echo "<option value=\"0\"";
            if ($document->getLinkedGISfeature() == '') echo " selected";
            echo ">-" . getMLtext('none') . "-</option>";
			foreach($features as $feat) {
				echo "<option value=\"".$feat->getFID()."\"";
				if($feat->getFID() == $document->getLinkedGISfeatureID())
					echo " selected";
				echo ">". $feat->getDisplayText() ."</option>";	
			}
        ?>
        </select>
      </td>
