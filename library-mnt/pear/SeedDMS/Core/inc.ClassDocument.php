 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_Document extends SeedDMS_Core_Object { /* begin function */
	/**
	...
	protected $_sequence;
    /**
	 * @var integer ID of linked GIS layer, if applicable
	 */
    protected $_linkedgislayer;
    /**
	 * @var string FID of linked GIS layer feature, if applicable
	 */
    protected $_linkedgisfeature;

	function SeedDMS_Core_Document($id, $name, $comment, $date, $expires, $ownerID, $folderID, $inheritAccess, $defaultAccess, $locked, $keywords, $sequence, $linkedgislayer = 0, $linkedgisfeature = '') { /* begin function */
        ...
        $this->_linkedgislayer = $linkedgislayer; //RR added
        $this->_linkedgisfeature = $linkedgisfeature; //RR added
	 ...
	 */
	function getDir() { /* begin function */
	 ...
	 */
	function setComment($newComment) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setKeywords($newKeywords) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function setCategories($newCategories) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getFolder() { /* begin function */
	 ...
	 */
	function setFolder($newFolder) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getOwner() { /* begin function */
	 ...
	 */
	function setOwner($newOwner) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function getDefaultAccess() { /* begin function */
	...
	function setDefaultAccess($mode) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function setInheritAccess($inheritAccess) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function expires() { /* begin function */
	 ...
	 */
	function getExpires() { /* begin function */
		if (intval($this->_expires) == 0)
			return false;
		else
	 ...
	 */
	function setExpires($expires) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function hasExpired() { /* begin function */
	 ...
	 */
	function verifyLastestContentExpriry(){ /* begin function */
	 ...
	 * @return boolean true if operation was successful otherwise false
	 */
	function setLocked($falseOrUser) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getLockingUser() { /* begin function */
		if (!$this->isLocked())
			return false;
	...
	function setSequence($seq) { /* begin function */
		$db = $this->_dms->getDB();
    ...
    //RR added
    /**
     * Get the aggregate document rating, or 0 if no ratings recorded for that document
     * 
    ...
    function getRating() {
		...
		$queryStr = "SELECT sum(rating) as doc_rating from tbldocumentrating GROUP BY documentid HAVING documentid = " . $this->_id;		
        $resArr = $db->getResultArray($queryStr);
		if (is_bool($resArr) && !$resArr)
			return -1;
        if (!isset($resArr[0])) return 0;
        $res = $resArr[0]; //should only be one row
        if (isset($res["doc_rating"])) { //has been rated
            return $res["doc_rating"];
        } else {
            return 0; //default rating 
        }
    }
    /**
     * Get the document rating by a specific user, or 0 if no rating recorded for that document
     * 
     * @return integer (-1 if error)
     */
    function getUsersRating($userid) {
		...
		$queryStr = "SELECT rating as doc_rating from tbldocumentrating WHERE documentid = " . $this->_id . " AND userid = " . $userid;		
        $resArr = $db->getResultArray($queryStr);
		if (is_bool($resArr) && !$resArr)
			return -1;
        if (!isset($resArr[0])) return 0;
    ...
    function getLinkedGISlayerID() {
        if ($this->_linkedgislayer)  return $this->_linkedgislayer;
        return 0;
    }
    function getLinkedGISlayer() {
        if ($this->_linkedgislayer) {
            $db = $this->_dms->getDB();

            $queryStr = "SELECT displayname from tblgislayer WHERE id = " . $this->_linkedgislayer;		
            $resArr = $db->getResultArray($queryStr);
            if (is_bool($resArr) && !$resArr)
                return -1;
            if (!isset($resArr[0])) return 0;
    ...
    function getLinkedGISfeatureID() {
        if ($this->_linkedgisfeature) return $this->_linkedgisfeature;
        return '';
    }
    function getLinkedGISfeature() {
        if ($this->_linkedgisfeature) {
            $db = $this->_dms->getDB();

            $queryStr = "SELECT attributes_concat, description_text from tblgislayer_feature WHERE (gislayer_id = " . $this->_linkedgislayer . " AND fid = '" . $this->_linkedgisfeature . "')";		
            $resArr = $db->getResultArray($queryStr);
            if (is_bool($resArr) && !$resArr)
                return -1;
            if (!isset($resArr[0])) return 0;
            ...
            if ($res["description_text"] > '') return $res["description_text"];
            //otherwise return all
            $formatted = str_replace(";","<br/>",$res["attributes_concat"]);
            return $formatted;
        } else {
            return '';
        }
    }
    function setLinkedGISlayer($layerid) {
        $db = $this->_dms->getDB();

        if ($layerid != $this->_linkedgislayer) {
            $queryStr = "UPDATE tblDocuments set linkedgislayer = " . $layerid . ", linkedgisfeature = '' WHERE id = " . $this->_id;
            if (!$db->getResult($queryStr))
                return false;
            $this->_linkedgislayer = $layerid;
        }
		return true;
    }
    function setLinkedGISfeature($fid) {
        $db = $this->_dms->getDB();

		$queryStr = "UPDATE tblDocuments set linkedgisfeature = '" . $fid . "' WHERE id = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;
		
		$this->_linkedgisfeature = $fid;
		return true;
    }
    //RR added end
	 ...
	 * @return boolean true if operation was successful otherwise false
	 */
	function clearAccessList() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getAccessList($mode = M_ANY, $op = O_EQ) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 *        user
	 */
	function addAccess($mode, $userOrGroupID, $isUser) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 *        user
	 */
	function changeAccess($newMode, $userOrGroupID, $isUser) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function removeAccess($userOrGroupID, $isUser) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getAccessMode($user) { /* begin function */
		if(!$user)
			return M_NONE;
		...
		if ($user->isGuest()) {
			...
			//RR change to make guest users not see unreleased docs.
			$lc=$this->getLatestContent();
			if ($lc) {
				$st = $lc->getStatus();
				if ($st["status"] != S_RELEASED) return M_NONE;
			} else { 
				return M_NONE;
			}
			//RR end
...
//					return $groupAccess->getMode();
		...
		if($result)
			return $result;
                ...
                //RR: admins and guests have already been dealt with
                if ($user->getID() != $this->_ownerID) return M_READ; /* RR so users can only edit their own docs */
	 ...
	 * @return integer access mode
	 */
	function getGroupAccessMode($group) { /* begin function */
	 ...
	 */
	function getNotifyList($type=0) { /* begin function */
	 ...
	 */
	function addNotify($userOrGroupID, $isUser) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function removeNotify($userOrGroupID, $isUser, $type=0) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function addContent($comment, $user, $tmpFile, $orgFileName, $fileType, $mimeType, $reviewers=array(), $approvers=array(), $version=0, $attributes=array(), $workflow=null) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getContent() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getContentByVersion($version) { /* begin function */
	...
	function getLatestContent() { /* begin function */
	...
	function removeContent($version) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 *         an error.
	 */
	function getDocumentLink($linkID) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getDocumentLinks($publiconly=false, $user=null) { /* begin function */
	 ...
	 * @return array list of objects of class SeedDMS_Core_DocumentLink
	 */
	function getReverseDocumentLinks($publiconly=false, $user=null) { /* begin function */
			$db = $this->_dms->getDB();
	...
	function addDocumentLink($targetID, $userID, $public) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function removeDocumentLink($linkID) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function getDocumentFile($ID) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function getDocumentFiles() { /* begin function */
	...
	function addDocumentFile($name, $comment, $user, $tmpFile, $orgFileName,$fileType, $mimeType ) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function removeDocumentFile($ID) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getApproversList() { /* begin function */
	 ...
	 */
	function getReadAccessList($listadmin=0, $listowner=0) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getFolderList() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function repair() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getUsedDiskSpace() { /* begin function */
		$db = $this->_dms->getDB();
 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_DocumentContent extends SeedDMS_Core_Object { /* begin function */
	 ...
	 */
	function verifyStatus($ignorecurrentstatus=false, $user=null) { /* begin function */
	...
	function SeedDMS_Core_DocumentContent($id, $document, $version, $comment, $date, $userID, $dir, $orgFileName, $fileType, $mimeType, $fileSize=0, $checksum='') { /* begin function */
		parent::__construct($id);
	...
	function setDate($date = false) { /* begin function */
	...
	function getFileSize() { /* begin function */
	 ...
	 */
	function setFileSize() { /* begin function */
	...
	function getChecksum() { /* begin function */
	 ...
	 */
	function setChecksum() { /* begin function */
	 ...
	 */
	function convert() { /* begin function */
	 ...
	 */
	function viewOnline() { /* begin function */
	...
	function wasConverted() { /* begin function */
	 ...
	 * This function is deprecated
	 */
	function getURL() { /* begin function */
	 ...
	 */
	function getStatus($limit=1) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function getStatusLog($limit=0) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function setStatus($status, $comment, $updateUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 * @return integer mode
	 */
	function getAccessMode($u) { /* begin function */
		if(!$this->_workflow)
			$this->getWorkflow();
	 ...
	 */
	function getReviewStatus($limit=1) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function getApprovalStatus($limit=1) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function addIndReviewer($user, $requestUser, $listadmin=false) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function addGrpReviewer($group, $requestUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function setReviewByInd($user, $requestUser, $status, $comment) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 * @return integer new review log id
	 */
	function setReviewByGrp($group, $requestUser, $status, $comment) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function addIndApprover($user, $requestUser, $listadmin=false) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function addGrpApprover($group, $requestUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function setApprovalByInd($user, $requestUser, $status, $comment) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function setApprovalByGrp($group, $requestUser, $status, $comment) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function delIndReviewer($user, $requestUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function delGrpReviewer($group, $requestUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function delIndApprover($user, $requestUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	...
	function delGrpApprover($group, $requestUser) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 * @param object $state
	 */
	function setWorkflowState($state) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function getWorkflowState() { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function setWorkflow($workflow, $user) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 *         or false in case of error, e.g. the version has not a workflow
	 */
	function getWorkflow() { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function rewindWorkflow() { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 *         or false in case of error
	 */
	function removeWorkflow($user, $unlink=false) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function getParentWorkflow() { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 * @param object $subworkflow
	 */
	function runSubWorkflow($subworkflow) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function returnFromSubWorkflow($user, $transition=null, $comment='') { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function triggerWorkflowTransitionIsAllowed($user, $transition) { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function executeWorkflowTransitionIsAllowed($transition) { /* begin function */
		if(!$this->_workflow)
			$this->getWorkflow();
	 ...
	 */
	function triggerWorkflowTransition($user, $transition, $comment='') { /* begin function */
		$db = $this->_document->_dms->getDB();
...
//			echo "run post function of transition ".$transition->getID()."<br />";
	 ...
	 *         false if not
	 */
	function enterNextState($user, $nextstate) { /* begin function */
...
//						echo "transition not ready for process now<br />";
	 ...
	 */
	function getWorkflowLog($transition = null) { /* begin function */
		$db = $this->_document->_dms->getDB();
...
*/
	 ...
	 * @return array list of operations
	 */
	function getLastWorkflowTransition() { /* begin function */
		$db = $this->_document->_dms->getDB();
	 ...
	 */
	function needsWorkflowAction($user) { /* begin function */
 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_DocumentLink { /* begin function */
	/**
 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_DocumentFile { /* begin function */
	/**
 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_AddContentResultSet { /* begin function */
	...
	function SeedDMS_Core_AddContentResultSet($content) { /* begin function */
	...
	function addReviewer($reviewer, $type, $status) { /* begin function */
	...
	function addApprover($approver, $type, $status) { /* begin function */
	...
	function setStatus($status) { /* begin function */
	...
	function getStatus() { /* begin function */
		return $this->_status;
	} /* end function */
	...
	function getReviewers($type) { /* begin function */
	...
	function getApprovers($type) { /* begin function */
		if (strcasecmp($type, "i") && strcasecmp($type, "g")) {
			return false;
		}
		if (!strcasecmp($type, "i")) {
