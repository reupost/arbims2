...
class SeedDMS_Core_User {
    ...
    //RR added
    /**
	 * @var boolean true if user receives monthly bulletin
	 *
	 * @access protected
	 */
	var $_isBulletin;
    //RR added
    /**
	 * @var boolean true if user account linked to ARBIMS portal
	 *
	 * @access protected
	 */
	var $_isLinkedAccount;
    //RR added
    /**
	 * @var boolean true if user account linked to ARBIMS portal
	 *
	...
	function SeedDMS_Core_User($id, $login, $pwd, $fullName, $email, $language, $theme, $comment, $role, $isHidden=0, $isDisabled=0, $pwdExpiration='', $loginFailures=0, $quota=0, $bulletin = 1, $linkedaccount = 0, $mastermoderator = 0) {
		$this->_id = $id;
		$this->_login = $login;
		$this->_pwd = $pwd;
		...
		$this->_quota = $quota;
        $this->_isBulletin = $bulletin; //RR added
        $this->_isLinkedAccount = $linkedaccount; //RR added
        $this->_isMasterModerator = $mastermoderator; //RR added
		$this->_dms = null;
	}
	...
	function setLogin($newLogin) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setFullName($newFullName) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setPwd($newPwd) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setPwdExpiration($newPwdExpiration) { /* begin function */
		$db = $this->_dms->getDB();

		//RR fix for invalid datetime ''
		if ($db->qstr($newPwdExpiration) != "''") {
			$queryStr = "UPDATE tblUsers SET pwdExpiration =".$db->qstr($newPwdExpiration)." WHERE id = " . $this->_id;
		} else {
			$queryStr = "UPDATE tblUsers SET pwdExpiration = DEFAULT(pwdExpiration) WHERE id = " . $this->_id;
		}
		$res = $db->getResult($queryStr);
		if (!$res)
			return false;
	...
	function setEmail($newEmail) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setLanguage($newLanguage) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setTheme($newTheme) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setRole($newrole) { /* begin function */
		$db = $this->_dms->getDB();
    ...
    //RR HACK added for 'super-admin'-type control over sensitive site settings
    //do not show these unless the super user has actually logged in (not a substitute user)
    function isSuperAdmin() { 
        global $session;
        if($session->getSu()) return false;
        return ($this->_fullName == "Reuben Roberts"); //HACK hard-coded 
    }
    
	function setAdmin($isAdmin) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setGuest($isGuest) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setHidden($isHidden) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setDisabled($isDisabled) { /* begin function */
		$db = $this->_dms->getDB();
    ...
    //RR added
    function isBulletin() { return $this->_isBulletin; }

	function setBulletin($isBulletin) { /* begin function */
		$db = $this->_dms->getDB();

		$isBulletin = ($isBulletin) ? 1 : 0;
		$queryStr = "UPDATE tblUsers SET bulletin = " . $isBulletin . " WHERE id = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;

		$this->_isBulletin = $isBulletin;
		return true;
	}	 /* end function */
    
    function isLinkedAccount() { return $this->_isLinkedAccount; }

	function setLinkedAccount($isLinkedAccount) { /* begin function */
		$db = $this->_dms->getDB();

		$isLinkedAccount = ($isLinkedAccount) ? 1 : 0;
		$queryStr = "UPDATE tblUsers SET linkedaccount = " . $isLinkedAccount . " WHERE id = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;

		$this->_isLinkedAccount = $isLinkedAccount;
		return true;
	}	 /* end function */
    
         function isMasterModerator() { return $this->_isMasterModerator; }

	function setMasterModerator($isMasterModerator) { /* begin function */
		$db = $this->_dms->getDB();

		$isMasterModerator = ($isMasterModerator) ? 1 : 0;
		$queryStr = "UPDATE tblUsers SET mastermoderator = " . $isMasterModerator . " WHERE id = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;

		$this->_isMasterModerator = $isMasterModerator;
		return true;
	}	 /* end function */
        
	function addLoginFailure() { /* begin function */
		$db = $this->_dms->getDB();
	...
	function clearLoginFailures() { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setQuota($quota) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function remove($user, $assignToUser=null) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function joinGroup($group) { /* begin function */
	 ...
	 */
	function leaveGroup($group) { /* begin function */
		if (!$group->isMember($this))
			return false;
	 ...
	 */
	function isMemberOfGroup($group) { /* begin function */
	 ...
	 */
	function hasImage() { /* begin function */
	 ...
	 */
	function getImage() { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setImage($tmpfile, $mimeType) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getDocuments() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return array list of documents
	 */
	function getDocumentsLocked() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getReviewStatus($documentID=null, $version=null) { /* begin function */
		$db = $this->_dms->getDB();
...
*/
	 ...
	 */
	function getApprovalStatus($documentID=null, $version=null) { /* begin function */
		$db = $this->_dms->getDB();
...
*/
   ...
   "SELECT `tblDocumentApprovers`.*, `tblDocumentApproveLog`.`status`, ".
	 ...
	 */
	function getWorkflowStatus($documentID=null, $version=null) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getMandatoryReviewers() { /* begin function */
		$db = $this->_dms->getDB();

		$queryStr = "SELECT * FROM tblMandatoryReviewers WHERE userID = " . $this->_id;
		
		$resArr = $db->getResultArray($queryStr);
                /* RR added: by default normal users must be reviewed by the admins */
                if (!count($resArr) & !$this->isAdmin()) { 
                    $queryStr = "SELECT " . $this->_id . " AS userID, id AS reviewerUserID, 0 AS reviewerGroupID FROM tblusers WHERE mastermoderator = 1";
                    $resArr = $db->getResultArray($queryStr);
                }
                /* RR added end */
		return $resArr;
	 ...
	 */
	function getMandatoryApprovers() { /* begin function */
		$db = $this->_dms->getDB();

		$queryStr = "SELECT * FROM tblMandatoryApprovers WHERE userID = " . $this->_id;
		$resArr = $db->getResultArray($queryStr);
                /* RR added: by default normal users must be approved by the admins */
                if (!count($resArr) & !$this->isAdmin()) { 
                    $queryStr = "SELECT " . $this->_id . " AS userID, id AS approverUserID, 0 AS approverGroupID FROM tblusers WHERE mastermoderator = 1";
                    $resArr = $db->getResultArray($queryStr);
                }
                /* RR added end */
		return $resArr;
	 ...
	 */
	function getMandatoryWorkflow() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function setMandatoryReviewer($id, $isgroup=false) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function setMandatoryApprover($id, $isgroup=false) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function setMandatoryWorkflow($workflow) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function delMandatoryReviewers() { /* begin function */
		$db = $this->_dms->getDB();
		$queryStr = "DELETE FROM tblMandatoryReviewers WHERE userID = " . $this->_id;
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function delMandatoryApprovers() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true on success, otherwise false
	 */
	function delMandatoryWorkflow() { /* begin function */
		$db = $this->_dms->getDB();
		$queryStr = "DELETE FROM tblWorkflowMandatoryWorkflow WHERE userid = " . $this->_id;
		if (!$db->getResult($queryStr)) return false;
		return true;
