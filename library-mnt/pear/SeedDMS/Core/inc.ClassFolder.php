...
class SeedDMS_Core_Folder extends SeedDMS_Core_Object {
	...
	function SeedDMS_Core_Folder($id, $name, $parentID, $comment, $date, $ownerID, $inheritAccess, $defaultAccess, $sequence) { /* begin function */
	 ...
	 */
	function getParent() { /* begin function */
	 ...
	 */
	function isSubFolder($subfolder) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function setParent($newParent) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function hasSubFolders() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getSubFolders($orderby="") { /* begin function */
		$db = $this->_dms->getDB();
...
//				$this->_subFolders[$i] = new SeedDMS_Core_Folder($resArr[$i]["id"], $resArr[$i]["name"], $resArr[$i]["parent"], $resArr[$i]["comment"], $resArr[$i]["owner"], $resArr[$i]["inheritAccess"], $resArr[$i]["defaultAccess"], $resArr[$i]["sequence"]);
	 ...
	 *         an error.
	 */
	function addSubFolder($name, $comment, $owner, $sequence, $attributes=array()) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getPath() { /* begin function */
	 ...
	 */
	function getFolderPathPlain() { /* begin function */
		$path="";
	 ...
	 */
	function isDescendant($folder) { /* begin function */
	 ...
	 */
	function hasDocuments() { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * of an error
	 */
	function hasDocumentByName($name) { /* begin function */
		$db = $this->_dms->getDB();
		if (isset($this->_documents)) {
			return count($this->documents);
		}
	 ...
	 */
	function getDocuments($orderby="") { /* begin function */
		$db = $this->_dms->getDB();
...
//				array_push($this->_documents, new SeedDMS_Core_Document($row["id"], $row["name"], $row["comment"], $row["date"], $row["expires"], $row["owner"], $row["folder"], $row["inheritAccess"], $row["defaultAccess"], isset($row["lockUser"])?$row["lockUser"]:NULL, $row["keywords"], $row["sequence"]));
	 ...
	 */
	function countChildren($user, $limit=10000) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function addDocument($name, $comment, $expires, $owner, $keywords, $categories, $tmpFile, $orgFileName, $fileType, $mimeType, $sequence, $reviewers=array(), $approvers=array(),$reqversion,$version_comment="", $attributes=array(), $version_attributes=array(), $workflow=null, $linkedgislayer = 0, $linkedgisfeature = '') { /* begin function */
		$db = $this->_dms->getDB();
		...
		$queryStr = "INSERT INTO tblDocuments (name, comment, date, expires, owner, folder, folderList, inheritAccess, defaultAccess, locked, keywords, sequence, linkedgislayer, linkedgisfeature) VALUES ".
					"(".$db->qstr($name).", ".$db->qstr($comment).", " . time().", ".(int) $expires.", ".$owner->getID().", ".$this->_id.",".$db->qstr($pathPrefix).", 1, ".M_READ.", -1, ".$db->qstr($keywords).", " . $sequence . ", " . $linkedgislayer . ",'" . $linkedgisfeature . "')"; //RR mod to include GIS layer
		if (!$db->getResult($queryStr)) {
			$db->rollbackTransaction();
			return false;
		}
...
//		else $res = $document->addContent($comment, $owner, $tmpFile, $orgFileName, $fileType, $mimeType, $reviewers, $approvers,$reqversion, $version_attributes, $workflow);
		...
		if($attributes) {            
			foreach($attributes as $attrdefid=>$attribute) {
                if (!is_array($attribute)) { //RR was trying to trim an array
		...
		if (!isset($this->_readAccessList)) {
			...
			if (!is_bool($resArr)) {
				foreach ($resArr as $row) {
					...
					if (!$listadmin && $user->isAdmin()) continue; 
					if (!$listowner && $user->getID() == $this->_ownerID) continue; //RR this logic is wrong.  Admin owns the folder, but not the file
