...
function doLogin() { /* begin function */
...
function doLogout() { /* begin function */
	global $app, $dms, $userobj, $session, $settings;
...
function setFullName() { /* begin function */
...
function setEmail($id) { /* begin function */
	global $app, $dms, $userobj;
...
function getLockedDocuments() { /* begin function */
	global $app, $dms, $userobj;
...
function getFolderParent($id) { /* begin function */
	global $app, $dms, $userobj;
	if($id == 0) {
...
function getFolderPath($id) { /* begin function */
	global $app, $dms, $userobj;
	if($id == 0) {
		echo json_encode(array('success'=>true, 'message'=>'id is 0', 'data'=>''));
		return;
...
function getFolderChildren($id) { /* begin function */
	global $app, $dms, $userobj;
	if($id == 0) {
...
function createFolder($id) { /* begin function */
	global $app, $dms, $userobj;
	if($id == 0) {
		echo json_encode(array('success'=>true, 'message'=>'id is 0', 'data'=>''));
		return;
...
function moveFolder($id) { /* begin function */
	global $app, $dms, $userobj;
	...
	if($mfolder) {
		if ($mfolder->getAccessMode($userobj) >= M_READ) {
			...
			if($folder = $dms->getFolder($folderid)) {
				if($folder->getAccessMode($userobj) > M_READWRITE) {/* RR was >= */
...
function deleteFolder($id) { /* begin function */
	global $app, $dms, $userobj;
	$mfolder = $dms->getFolder($id);
	if($mfolder) {
		if ($mfolder->getAccessMode($userobj) > M_READWRITE) { /* RR was >= */
			if($mfolder->remove()) {
				$app->response()->header('Content-Type', 'application/json');
				echo json_encode(array('success'=>true, 'message'=>'', 'data'=>''));
			} else {
...
function deleteDocument($id) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
	if($document) {
...
function moveDocument($id) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
	if($document) {
		if ($document->getAccessMode($userobj) >= M_READ) {
...
function getDocumentVersions($id) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
...
function getDocumentVersion($id, $version) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
...
function getDocumentFiles($id) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
...
function getDocumentFile($id, $fileid) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
...
function getDocumentLinks($id) { /* begin function */
	global $app, $dms, $userobj;
	$document = $dms->getDocument($id);
...
function getAccount() { /* begin function */
	global $app, $dms, $userobj;
	if($userobj) {
		$account = array();
 ...
 */
function doSearch() { /* begin function */
	global $app, $dms, $userobj;
