...
$LANG = array();
$MISSING_LANG = array();
//RR for shared language files
if ($settings->_sharedLangPath != "") {
    foreach(getLanguages() as $_lang) {
        if(file_exists($settings->_sharedLangPath . "/" . $_lang . "/lang.inc")) {
            include $settings->_sharedLangPath . "/" . $_lang . "/lang.inc";
            $LANG[$_lang] = $text;
        }
    }
} else { //RR end
...
function printReviewStatusText($status, $date=0) { /* begin function */
	if (is_null($status)) {
...
function getReviewStatusText($status, $date=0) { /* begin function */
	if (is_null($status)) {
...
function printApprovalStatusText($status, $date=0) { /* begin function */
	if (is_null($status)) {
		print getMLText("status_unknown");
	}
	else {
...
function getApprovalStatusText($status, $date=0) { /* begin function */
	if (is_null($status)) {
		return getMLText("status_unknown");
	}
	else {
...
function printOverallStatusText($status) { /* begin function */
	print getOverallStatusText($status);
} /* end function */

function getOverallStatusText($status) { /* begin function */
	if (is_null($status)) {
