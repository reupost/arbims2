...
class SeedDMS_Lucene_IndexedDocument extends Zend_Search_Lucene_Document {
	...
	public function __construct($dms, $document, $convcmd=null, $nocontent=false) {
		//RR modified various items below
		/* only running apache as httpd.exe instead of as a Windows service allowed these to work */
		/* still to test */
		...
		$_convcmd = array(
			...
			'text/plain' => 'cat "%s"', 
			'application/msword' => '"C:\inetpub\wwwroot\arbims\getwordtext.bat" "%s"',
			'application/vnd.openxmlformats-officedocument.wordprocessingml.document' => '"C:\inetpub\wwwroot\arbims\getwordtext.bat" "%s"',			
			'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' => '"C:\Program Files (x86)\PHP\extras\getwordtext.bat" "%s"',
			'application/vnd.openxmlformats-officedocument.presentationml.presentation' => '"C:\Program Files (x86)\PHP\extras\getwordtext.bat" "%s"',
		);
		if($convcmd) {
			$_convcmd = $convcmd;
		}
		...
		if($version && !$nocontent) {
			...
			if(isset($_convcmd[$mimetype])) {
				...
				if($content) {
					$content = utf8_encode($content); //RR added for Apache Tika content which otherwise fails silently
