 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_Attribute { /* begin function */
	/**
	 ...
	 */
	function SeedDMS_Core_Attribute($id, $obj, $attrdef, $value) { /* begin function */
		$this->_id = $id;
		$this->_obj = $obj;
 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_AttributeDefinition { /* begin function */
	/**
	 ...
	 */
	function SeedDMS_Core_AttributeDefinition($id, $name, $objtype, $type, $multiple, $minvalues, $maxvalues, $valueset, $regex) { /* begin function */
		$this->_id = $id;
		$this->_name = $name;
		$this->_type = $type;
	...
	function setName($name) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setObjType($objtype) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setType($type) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setMultipleValues($mv) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setMinValues($minvalues) { /* begin function */
		$db = $this->_dms->getDB();
	...
	function setMaxValues($maxvalues) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 */
	function getValueSet() { /* begin function */
	 ...
	 *         less than 2 chars
	 */
	function getValueSetAsArray() { /* begin function */
		if(strlen($this->_valueset) > 1)
			return explode($this->_valueset[0], substr($this->_valueset, 1)); //RR WHY trim off the first character?  It means you have to preface your list with the delimiter which is very unintuitive
		else
			return false;
	} /* end function */
	 ...
	 */
	function getValueSetValue($ind) { /* begin function */
		if(strlen($this->_valueset) > 1) {
	 ...
	 */
	function setValueSet($valueset) { /* begin function */
	/*
		$tmp = array();
	...
	*/
	 ...
	 * Get the separator
	 *
	 * @return string separator character, or false if error
	 */
    function getSeparator() {
        if(strlen($this->_valueset) > 1) 
            return substr($this->_valueset, 0, 1);
        return false;
    }
	 ...
	 */
	function getRegex() { /* begin function */
		return $this->_regex;
	} /* end function */
	 ...
	 */
	function setRegex($regex) { /* begin function */
		$db = $this->_dms->getDB();
	 ...
	 * @return boolean true if attribute definition is used, otherwise false
	 */
	function getStatistics($limit=0) { /* begin function */
		$db = $this->_dms->getDB();
