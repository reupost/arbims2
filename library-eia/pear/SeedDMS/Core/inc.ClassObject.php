 ...
 * @version    Release: 4.3.8
 */
class SeedDMS_Core_Object { /* begin function */
	/**
	...
	function SeedDMS_Core_Object($id) { /* begin function */
		$this->_id = $id;
		$this->_dms = null;
	} /* end function */
	 ...
	 */
	function getAttributes() { /* begin function */ 
		if (!$this->_attributes) {
			$db = $this->_dms->getDB();
	 ...
	 */
	function getAttributeValue($attrdef) { /* begin function */
		if (!$this->_attributes) {
			$this->getAttributes();
		}
	 ...
	 */
	function setAttributeValue($attrdef, $value) { /* begin function */
		$db = $this->_dms->getDB();
		if (!$this->_attributes) {
			$this->getAttributes();
		}
		if($attrdef->getMultipleValues() && is_array($value)) {
			...
			$value = array_filter($value); //RR remove empty elements
			$value = $sep.implode($sep, $value);
			//RR added for picking empty row in multi-select ==> delete old values
			if ($value == $sep) { 
				$this->removeAttribute($attrdef);
				return true;
			} //RR added end
		}
		if(!isset($this->_attributes[$attrdef->getId()])) {
	 ...
	 * @return boolean true if operation was successful, otherwise false
	 */
	function removeAttribute($attrdef) { /* begin function */
		$db = $this->_dms->getDB();
		if (!$this->_attributes) {
			$this->getAttributes();
		}
