...
class SeedDMS_Core_DatabaseAccess {
	 ...
	 */
	function TableList() { /* begin function */
		switch($this->_driver) {
			case 'mysql':
	 ...
	 */
	function SeedDMS_Core_DatabaseAccess($driver, $hostname, $user, $passw, $database = false) { /* begin function */
		$this->_driver = $driver;
	 ...
	 */
	function connect() { /* begin function */
		switch($this->_driver) {
			case 'mysql':
			case 'mysqli':
			case 'mysqlnd':
		...
		$this->_conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); //RR added debugging
		return true;
	} /* end function */
	 ...
	 */
	function ensureConnected() { /* begin function */
	 ...
	 */
	function qstr($text) { /* begin function */
	 ...
	 */
	function getResultArray($queryStr) { /* begin function */
		$resArr = array();
...
//		$res->Close();
	 ...
	 */
	function getResult($queryStr, $silent=false) { /* begin function */
	...
	function startTransaction() { /* begin function */
	...
	function rollbackTransaction() { /* begin function */
	...
	function commitTransaction() { /* begin function */
		if($this->_intransaction == 1) {
			$this->_conn->commit();
		}
		$this->_intransaction--;
	 ...
	 */
	function getInsertID() { /* begin function */
		return $this->_conn->lastInsertId();
	} /* end function */

	function getErrorMsg() { /* begin function */
		$info = $this->_conn->errorInfo();
		return($info[2]);
	} /* end function */

	function getErrorNo() { /* begin function */
	 ...
	 */
	function createTemporaryTable($tableName, $override=false) { /* begin function */
	 ...
	 */
	function getDateExtract($fieldname, $format='%Y-%m-%d') { /* begin function */
		switch($this->_driver) {
			case 'mysql':
