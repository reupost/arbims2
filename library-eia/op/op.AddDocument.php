...
//RR added
$gislayerid = 0;
if (isset($_POST["linkedgislayer"])) {
    $gislayerid = $_POST["linkedgislayer"]; 
    if (!is_numeric($gislayerid)) $gislayerid = 0;
}
$gisfeatureid = ''; 
if (isset($_POST["linkedgisfeature"])) {
    $gisfeatureid = $_POST["linkedgisfeature"];     
}
...
for ($file_num=0;$file_num<count($_FILES["userfile"]["tmp_name"]);$file_num++){
	...
	$res = $folder->addDocument($name, $comment, $expires, $user, $keywords,
                                ...
                                $gislayerid, $gisfeatureid); //RR mod to include GIS layer info
	...
	if (is_bool($res) && !$res) {
		...
		if($settings->_enableFullSearch) {
			...
			if($index) {
                ...
                //RR fix $settings->_convcmd is null; rather using $settings->_converters
				//$index->addDocument(new SeedDMS_Lucene_IndexedDocument($dms, $document, isset($settings->_convcmd) ? $settings->_convcmd : null, true));                
                $index->addDocument(new SeedDMS_Lucene_IndexedDocument($dms, $document, isset($settings->_converters) ? $settings->_converters : null)); //RR final 'true'param above meant only metadata indexed, not content
            }
		}
