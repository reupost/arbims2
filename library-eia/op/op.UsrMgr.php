...
if ($action == "adduser") {
    ...
    $isBulletin = (isset($_POST["isbulletin"]) && $_POST["isbulletin"]==1 ? 1 : 0); //RR added
    $isMasterModerator = (isset($_POST["ismastermoderator"]) && $_POST["ismastermoderator"]==1 ? 1 : 0); //RR added
	...
	$newUser = $dms->addUser($login, md5($pwd), $name, $email, $settings->_language, $settings->_theme, $comment, $role, $isHidden, $isDisabled, $pwdexpiration, $isBulletin, $isMasterModerator);
	if ($newUser) {
...
// modify user ------------------------------------------------------------
    ...
    if ($editedUser->isBulletin() != $isBulletin) //RR
		$editedUser->setBulletin($isBulletin);
    if ($editedUser->isMasterModerator() != $isMasterModerator) //RR
		$editedUser->setMasterModerator($isMasterModerator);
