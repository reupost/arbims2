...
$(document).ready( function() {
	...
	$(".chzn-select-deselect").chosen({allow_single_deselect:true});
        $("#linkedgislayer").chosen().change(function() 
            {                
                $('#linkedgisfeature').empty(); //remove all child nodes
                var newOption = $('<option value="0">-None-</option>');
                $('#linkedgisfeature').append(newOption);
                $.ajax({
                    type: 'GET',
                    url: '/data.map_feature.php',
                    data: { id : $("#linkedgislayer").chosen().val() },                
                    dataType: 'json',             
                    cache: false,
                    success: function(result) {                          
                        if (typeof result[0] !== 'undefined') {                                 
                            $.each(result, function(index, element) {                                                
                                var newOption = $('<option value="' + element['fid'] + '">' + element['descr'] + '</option>');
                                $('#linkedgisfeature').append(newOption);
                            });
                            $("#linkedgisfeature").trigger("liszt:updated");
                        }
                    }                                           
                });
            }
